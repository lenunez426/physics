...
In the figure below, Alice, who belongs to frame A, stands at rest on the street. She observes Bob, in Frame B, moving at speed $v_{BA}$ relative to her frame. A pelican $P$ moves relative to both frames.

\begin{center}%...Borrowed from Halliday & Resnick 4-6 Relative Motion
\begin{tikzpicture}[x=2cm,y=2cm]
    \draw (0,0) -- (0,1) node[above] {$y$} node[above=5mm] {Frame A};
    \draw (0,0) -- (3.5,0) node[right] {$x$};
    \begin{scope}[shift={(1,0.1)}]
        \draw (0,0) -- (0,0.9) node[above] {$y$} node[above=5mm] {Frame B};
        \draw (0,0) -- (3,0) node[right] {$x$}; 
        \draw[ultra thick,->] (0,0) -- (1,0) node[above] {$\vec{v}_{BA}$};
        \fill[lightgray] (0,0) circle (2pt);
        \draw (2,0) node[above=-4pt] {\tiny $|$} node [above=3pt] {$x_{PB}$};
        \draw[fill=lightgray] (2,0.75) circle (1.5mm) node[right=1.5mm] {$P$};
    \end{scope}
    \fill[lightgray] (0,0) circle (2pt) node[below=2pt,black] {0};
    \draw (1,0) node[below=-4pt] {\tiny $|$} node [below=3pt] {$x_{BA}$};
    \draw (3,0) node[below=-4pt] {\tiny $|$} node [below=3pt] {$x_{PA} = x_{PB} + x_{BA}$};
\end{tikzpicture}
\end{center}

Let the velocity of Frame B (Bob) relative to Frame A (Alice) be $\vec{v}_{BA}$. 

\begin{equation*}
    v_{PA} = v_{PB} + v_{BA}
\end{equation*}

This result generalizes for vectors in one dimension:
...
\begin{example}
    If Bob, from the figure above, moves at \SI{32}{m/s} away from Alice, and Alice measures particle P to be moving at \SI{45}{m/s} in the $-x$-direction, what is the particle's velocity relative to Frame B?
\end{example}

\textit{Solution:}
Equation \eqref{77nBP} yields

\begin{equation*}
    \SI{-45}{m/s}= v_{PB} + \SI{32}{m/s}
\end{equation*}
