...
\firstpageheader{{Name:\enspace\makebox[6cm]{\hrulefill}}\\Physics}{}{Review on Unit 10: Mechanical Waves\\}
\runningheader{Physics}{}{Review on Unit 7: Oscillations}
...
\question
A pulse is sent along a medium toward a boundary with a loose end.
...
\begin{tikzpicture}[x=8mm,y=1.2cm]
    ...
    \node[above=2mm] at (current bounding box.north) {Incident Wave};
\end{tikzpicture}
\hspace{2cm}
\begin{tikzpicture}[x=8mm,y=1.2cm]
    \ifprintanswers
        \draw[red,thick,domain=-3:2.85,samples=100] plot (\x,{exp(-(\x)^2)});
        \draw[<-,red] (-0.5,1.2) -- (0.5,1.2);
    \else
        \draw[red,thick,domain=-3:2.85,samples=100,opacity=0] plot (\x,{exp(-(\x)^2)});
        \draw[<-,red,opacity=0] (-0.5,1.2) -- (0.5,1.2);
    \fi
    \draw (3,1) -- (3,-1);
    \draw (2.85,0) arc (180:540:0.15 and 0.05);
    \node[above=2mm] at (current bounding box.north) {Reflected Wave};
\end{tikzpicture}
\end{center}

In the space above, draw the reflected wave. Then, below, explain how the reflected wave would look if instead the boundary was a fixed end.

\ifprintanswers
\textcolor{red}{If the end was fixed, the reflected wave would flip so that the pulse travels ``upside down''.}
\else
\fillwithlines{14mm}
\fi
