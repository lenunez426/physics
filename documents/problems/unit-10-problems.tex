...
\firstpageheader{{Name:\enspace\makebox[6cm]{\hrulefill}}\\Physics}{}{Unit 10: Mechanical Waves}
% \firstpageheader{Physics}{Unit 9: Oscillatory Motion}{Problems}
\runningheader{{Name:\enspace\makebox[6cm]{\hrulefill}}\\Physics}{}{Unit 10: Mechanical Waves}
...
\ifprintanswers
\textcolor{red}{A wave is a disturbance that carries energy through matter or space without transferring matter.}
\else
\fillwithlines{14mm}
\fi

\question 
When a wave travels through a medium, it transfers \fillin[energy][4cm]. However, a wave does not transfer \fillin[matter][4cm]. How is this demonstrated in a wave pool?

\ifprintanswers
\textcolor{red}{Dropping a piece of rock in a pool produces ripples in all directions (energy transfer), but the water (matter) level remains the same.}
\else
\fillwithlines{14mm}
\fi
...
\ifprintanswers
\textcolor{red}{Medium is any substance through which the waves travel, like air, rock, rope, spring, and water.}
\else
\fillwithlines{14mm}
\fi
...
\ifprintanswers
\textcolor{red}{Mechanical waves travel through a physical medium (water waves, sound waves, seismic waves); electromagnetic waves in addition, can travel through a vacuum (radio waves, microwaves, x-rays).}
\else
\fillwithlines{21mm}
\fi

\question 
In a transverse wave the particles move \fillin[perpendicular][4cm]\  relative to the direction the wave moves. Draw a diagram of a transverse wave. Label the direction the particles move along with the direction the wave moves. Diagram:


\begin{solutionorbox}[3cm]
\begin{center}
\begin{tikzpicture}
    \draw[thick,domain=0:10,samples=100] {plot(\x,{sin(deg(2*pi*\x/(10/3)))})};
    \draw[<->] (-0.5,-1) -- (-0.5,1) node[above,pos=0.5,rotate=90] {disturbance};
    \draw[->] (0.83,1.2) -- ++(1.5,0) node[above,pos=0.5] {propagation};
\end{tikzpicture}
\end{center}
\end{solutionorbox}
...
\question 
In a longitudinal wave the particles move \fillin[parallel][4cm]\ relative to the direction the wave moves. Draw a diagram of a longitudinal wave. Label the direction the particles move along with the direction the wave moves. Diagram:                   
...
\ifprintanswers
\textcolor{red}{A single bump or disturbance that travels through a medium.}
\else
\fillwithlines{14mm}
\fi
...
\begin{center}
\begin{tabular}{lll}
     amplitude \hspace{5mm} & wavelength (transverse) \hspace{5mm} &  \\
...
\begin{tikzpicture}[y=1.5cm]
    ...
    \draw[densely dashed] (0,0) -- (10.5,0);
    \begin{scope}
        \ifprintanswers \color{red} \else \color{white} \fi
        \draw[<-,shift={(0.05,0.05)}] (0.83,1) -- ++(0.8,0.5) node[above] {crest};
        \draw[<-,shift={(-0.01,-0.05)}] (2.5,-1) -- ++(-0.8,-0.5) node[below] {trough};
        \draw[<->,shift={(0,0.05)}] (4.17,1) -- (7.5,1) node[above,pos=0.5] {wavelength};
        \draw[<->] (7.5,0.98) -- (7.5,0.02) node[below] {amplitude};
    \end{scope}
\end{tikzpicture}
\end{center}
...
\ifprintanswers
\textcolor{red}{The time it takes for one complete cycle.}
\else
\fillwithlines{14mm}
\fi
...
\begin{parts}
\part Earth orbiting the Sun: \fillin[365 days] 
\part Small hand of a clock: \fillin[12 hours]
\end{parts}

\question
The following graph represents the position of a mouse on a bungee cord. Answer the questions below using the graph.
...
\begin{tikzpicture}
    ...
    \begin{scope}[shift={(12,1)}]
        \draw[decorate,decoration={coil,segment length=1.5mm,post length=2mm,pre length=1mm,amplitude=3mm}] (0,3) -- (0,0);
        \node[shift={(0,-2mm)}] at (0,0) {\twemoji[height=1cm]{mouse face}};
    \end{scope}
\end{tikzpicture}
\end{center}

\begin{parts}
\part What point is 1 full cycle after Point A? \fillin[E][2cm]

\part What point is a half-cycle after Point G? \fillin[I][2cm]

\part If C represents the lowest point of the mouse's bungee cycle, then where is the mouse at D? 

\ifprintanswers 
\textcolor{red}{The mouse is at the equilibrium position.}
\else \fillwithlines{7mm} \fi

\part What is the period of the mouse's oscillations? How is this period determined?

\ifprintanswers 
\textcolor{red}{The period is 1 second. The period is determined by finding one wave cycle on the graph.}
\else \fillwithlines{14mm} \fi
...
\ifprintanswers 
\textcolor{red}{The number of oscillations per second.}
\else \fillwithlines{7mm} \fi
...
What are the units used to measure period? \fillin[seconds (s)][4cm]
...
What are the units used to measure frequency? \fillin[Hertz (Hz), or per seconds ($\mathrm{s^{-1}}$)][4cm]

\question
How are frequency and period related mathematically? Write the relationship as a formula.

\ifprintanswers
\textcolor{red}{$\displaystyle T = \frac{1}{f} \qquad f = \frac{1}{T}$}
\else
\fillwithlines{7mm}
\fi
...
\begin{equation*}
    T = \frac{\text{total time}}{\text{number of cycles}} = \frac{\SI{5}{s}}{\SI{8}{cycles}} = \boxed{\SI{0.625}{s}}
\end{equation*}
\end{solutionorbox}
...
\begin{solutionorbox}[1cm]
\begin{equation*}
    f = \frac{\text{number of cycles}} {\text{total time}}= \frac{\SI{1}{cycle}}{\SI{30}{s}} = \boxed{\SI{0.033}{Hz}}
\end{equation*}
\end{solutionorbox}
...
\question 
How is the energy of a wave related to the wave amplitude?

\ifprintanswers 
\textcolor{red}{Energy is proportional to amplitude. The greater the energy, the bigger the amplitude.}
\else \fillwithlines{14mm} \fi

\question
How is the relation between energy and amplitude evident in wave surfing?

\ifprintanswers 
\textcolor{red}{Larger waves cause more accidents by transferring more energy.}
\else \fillwithlines{14mm} \clearpage \fi


\question
If a simple harmonic oscillator is at the  equilibrium position, work must be done on the system in order to add energy to the system. Which of the following graphs represent the greatest amount of energy added to the system? Explain why. 
...
\ifprintanswers 
\textcolor{red}{Graph 1, because its amplitude is greater than that of Graph 2.}
\else \fillwithlines{14mm} \fi

\question 
How does the velocity of a wave relate to the frequency of a wave? The figures below show systems of waves set up in strings, fixed on both ends, under tension. Each set was produced in 1.0 seconds. All the strings have identical tension and length. The variables are amplitude and the number of waves present. Rank the wavelength, frequency and velocity of each wave from least to greatest.

\begin{center}
\begin{tikzpicture}[y=0.6mm,x=12cm]
    \draw[domain=0:1,samples=100,thick] plot(\x,{15*sin(deg(2*pi*1.5*\x))}) node[left,pos=0] {A};
    \draw[domain=0:1,samples=100,densely dashed,lightgray] plot(\x,{-15*sin(deg(2*pi*1.5*\x))});
\end{tikzpicture}
\vspace{5mm}

\begin{tikzpicture}[y=0.6mm,x=12cm]
    \draw[domain=0:1,samples=100,thick] plot(\x,{12*sin(deg(2*pi*2*\x))}) node[left,pos=0] {B};
    \draw[domain=0:1,samples=100,densely dashed,lightgray] plot(\x,{-12*sin(deg(2*pi*2*\x))});
\end{tikzpicture}
\vspace{5mm}

\begin{tikzpicture}[y=0.6mm,x=12cm]
    \draw[domain=0:1,samples=100,thick] plot(\x,{10*sin(deg(2*pi*2.5*\x))}) node[left,pos=0] {C};
    \draw[domain=0:1,samples=100,densely dashed,lightgray] plot(\x,{-10*sin(deg(2*pi*2.5*\x))});
\end{tikzpicture}
\vspace{5mm}

\begin{tikzpicture}[y=0.6mm,x=12cm]
    \draw[domain=0:1,samples=100,thick] plot(\x,{16*sin(deg(2*pi*3.5*\x))}) node[left,pos=0] {D};
...
\begin{tabular}{lcccccc}
    \textbf{Wavelength:} & \textit{least} & \fillin[D][3em] & \fillin[C][3em] & \fillin[B][3em] & \fillin[A][3em] & \textit{greatest} \\
    \textbf{Frequency:} & \textit{least} & \fillin[A][3em] & \fillin[B][3em] & \fillin[C][3em] & \fillin[D][3em] & \textit{greatest} \\
    \ifprintanswers
    \textbf{Velocity:} & \textit{least} 
    & \fillin[][3em] 
    & \fillin[][3em] 
    & \rlap{\textcolor{red}{\hspace{-2.8cm} All waves have the same velocity.}}\fillin[][3em] 
    & \fillin[][3em] 
    & \textit{greatest} \\
    \else
    \textbf{Velocity:} & \textit{least} & \fillin[][3em] & \fillin[][3em] & \fillin[][3em] & \fillin[][3em] & \textit{greatest} \\
    \fi
\end{tabular}
\end{center}
\begin{solution}
    All waves have the same velocity because the string tension is the same for all waves, and wave speed is
...
\question
What is the period, in seconds, and frequency, in hertz, of a heart that beats 60 times in one minute?
...
\question
At the beach today, the waves are \SI{0.34}{m} apart and have a frequency of $1/10\,\text{Hz}$. Find the speed of the waves.
...
\subsection*{Lab: Generating Transverse Waves}
...
\begin{mdframed}
\centering
\textbf{Lab Procedures}
\smallskip

\begin{enumerate}[itemsep=0pt,topsep=0pt]
    \item Stretch the ends of the coil to a distance of 5 to 6 meters. Record the distance stretched below:

    \begin{center}
        \textbf{Distance coil was stretched}: \fillin[5.0]\ meters
    \end{center}
    
    \item Oscillate the coil to start a standing wave that is $\frac{1}{2}$ wavelength long.
    \item When you have a smooth wave, measure the time it takes to complete 10 cycles, and record your data in table below. \textit{Recall:} Moving your hand back and forth is one cycle. 
    \item Repeat steps 2 and 3 for 1, 1.5, 2, 2.5, and 3.0 wavelengths.
    \item Use the period formula $T = \frac{1}{f}$ and wave speed formula $v = f \lambda$ to fill the table below.
\end{enumerate}
\end{mdframed}
...
\begin{tabular}{|c|c|c|c|c|c|c|}
    \hline
    \thead{\textbf{\# of}\\ \textbf{Waves}} & \thead{\textbf{Diagram of wave}} & \thead{\textbf{Time (s) for}\\ \textbf{10 cycles}} & \thead{\textbf{Frequency}\\ \textbf{(cycles/s)}} & \thead{\textbf{Period (s)}} & \thead{\textbf{Wavelength}\\\textbf{(m)}} & \thead{\textbf{Velocity}\\ \textbf{(m/s)}}\\ \hline
...
\begin{equation*}
    f = \frac{\text{10 cycles}}{\text{time}} 
      ...
      = \boxed{\rule{0pt}{3ex} \ifprintanswers \color{red} \else \color{white} \fi \qquad \SI{1.23}{Hz} \qquad}
\end{equation*}
}
...
\begin{equation*}
    ...
    = \boxed{\rule{0pt}{3ex} \ifprintanswers \color{red} \else \color{white} \fi \qquad \SI{0.81}{s} \qquad}
\end{equation*}
}
...
{\large
\begin{equation*}
    \lambda = \frac{\text{distance stretched}}{\text{number of waves}} = \frac{\ifprintanswers \color{red} \else \color{white} \fi \SI{5.00}{m}}{\ifprintanswers \color{red} \else \color{white} \fi \qquad \text{2 waves} \qquad} = \boxed{\rule{0pt}{3ex} \ifprintanswers \color{red} \else \color{white} \fi \qquad \SI{2.50}{m} \qquad}
\end{equation*}
}
...
\question
According to the data, what is the proportional relationship between wavelength and frequency?\\[1ex] Wavelength is \fillin[inversely proportional][8cm] the frequency.

% \begin{randomizechoices}
%     \correctchoice inversely proportional to
%     \choice proportional to 
%     \choice proportional to the square of
%     \choice inversely proportional to the
...
\question
What is the slope of the wavelength ($\lambda$) vs period ($T$) graph? Include correct units. 
%What is the significance (meaning) of this slope? \textit{Hint:} Look at the units.

\begin{solutionorbox}[2cm]
According to the best fit line (see Desmos), the slope is \SI{7.886}{m/s}.
\end{solutionorbox}

\question
(a) Write the linear equation ($y = mx + b$) for the wavelength vs period graph, but do not use $x$ and $y$ in your equation. Instead, use $\lambda$ and $T$, and include units for your slope. (b) What physical quantity does the slope represent? \textit{Hint:} Look at the units.

\ifprintanswers
{\color{red} (a) \quad $\lambda = (\SI{7.886}{m/s})\, T - \SI{0.188}{m}$ \quad (b) Slope represents the speed of the wave, since units are meters per second.
}
\else
\fillwithlines{14mm}
\fi
...
\ifprintanswers
{\color{red} $\bar{v} = \SI{7.58}{m/s}$. While there is some variation in velocities, the overall trend is constant.
...
\begin{align*}
    A &= \SI{9}{cm} \\[1ex]
    \lambda &= \SI{20}{cm} \\[1ex]
    T &= \frac{1}{f} = \frac{1}{\SI{25.0}{Hz}} = \SI{0.040}{s} \\[1ex]
    v &= f\lambda = (\SI{25}{Hz})(\SI{20}{cm}) = \SI{500}{cm/s}
\end{align*}
}
\end{minipage}
...
\end{tikzpicture}
\hspace{1cm}
\begin{tikzpicture}[y=1cm,x=7mm]
    \draw[thick,domain=0:10,samples=200] {plot(\x,{sin(deg(2*pi*\x/4))})};
    \draw[densely dashed] (0,0) -- (10,0);
    \draw[<->,shift={(0,0.07)}] (1,1) -- (5,1) node[above,pos=0.5] {$\lambda$};
    \draw[<->] (9,0) -- (9,1) node[right,pos=0.4] {X};
...
\begin{axis}[height=4.0cm,width=8cm,
    ...
    \addplot[domain=-6:6,samples=120,thick, green!60!black] {4*exp(-(x-3)^2)};
    \fi
...
\begin{axis}[height=4.0cm,width=8cm,
    ...
    \addplot[domain=-6:6,samples=120,thick, green!60!black] {4*exp(-(x-1.5)^2)};
    \fi
...
\begin{axis}[height=4.0cm,width=8cm,
    ...
    \addplot[domain=-6:6,samples=120,thick, green!60!black] {4*exp(-(x-0.75)^2)};
    \fi
...
\begin{axis}[height=4.0cm,width=8cm,
    ...
    \addplot[domain=-6:6,samples=120,thick, green!60!black] {4*exp(-(x)^2)};
    \fi
...
\part The function $F_3$ depicts the wave phenomenon called \fillin[constructive][4cm]\ interference.
\end{parts}
...
\begin{axis}[height=5.0cm,width=8cm,
    ...
    \addplot[domain=-6:6,samples=120,thick, green!60!black] {-4*exp(-(x-1)^2)};
    \fi
...
\begin{axis}[height=5.0cm,width=8cm,
    ...
    \addplot[domain=-6:6,samples=120,thick, green!60!black] {-4*exp(-(x-0.3)^2)};
    \fi
