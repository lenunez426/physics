...
\firstpageheader{{Name:\enspace\makebox[6cm]{\hrulefill}}\\{Physics}}{}{Unit 8:\\Conservation in Mechanical Systems}
\runningheader{{Name:\enspace\makebox[6cm]{\hrulefill}}}{}{}
% \runningheader{Physics}{}{Unit 8}


\subsection*{Desmos: Gravitational Potential Energy}

\begin{questions}
\question
\textit{Warm-up}. Recall Newton's universal law of gravitation, which calculates the force of attraction:

\begin{equation*}
    F_g = \frac{G m_1 m_2}{r^2}
\end{equation*}

where $m_1$ and $m_2$ are masses in kilograms, $r$ is the distance in meters between the centers of mass, and $G = \SI{6.67e-11}{N\cdot m^2/kg^2}$ is the universal gravitational constant.

\textbf{\underline{Calculate}} the gravitational force by Earth on a 1.0 kilogram object resting on the surface of Earth. Earth's mass is \SI{5.97e24}{kg}, and the separation distance between Earth and object is Earth's radius, \SI{6.378e6}{m}. \textbf{\underline{Round}} your answer to the nearest whole number.

\begin{solutionorbox}[1.5cm]

\begin{equation*}
    F_g = \frac{G m_1 m_2}{r^2} = \frac{(\SI{6.67e-11}{N\cdot m^2/kg})(\SI{1.00}{kg})(\SI{5.97e24}{kg})}{(\SI{6.378e6}{m})^2} = \SI{9.78}{N} \approx \boxed{\SI{10}{N}}
\end{equation*}
\end{solutionorbox}

\question
\textit{Intro to GPE}. When two masses are separated by some distance, energy is stored in the configuration. This stored energy is called gravitational potential energy. The equation and figure below show that raising a bowling ball ($m_1$) above the surface of the Earth ($m_2$) stores energy ($E_g$).
...
\begin{minipage}{0.4\textwidth}
\centering
\begin{tikzpicture}[x=7mm,y=7mm,transform shape]
    \draw (0,0) node[opacity=0.3] {\twemoji[width=3.5cm]{globe showing Americas}};
    \draw (0,0) circle (2.5) node[right=2.5] {$m_2$};
...
Go to \href{https://www.desmos.com/scientific}{desmos.com/scientific}. Type the following variables in each cell:
...
\begin{tabular}{|l|l|}
    \hline
    \thead{\textbf{Type this in Desmos}} & \thead{\textbf{What it means}} \\ \hline
    $G = \num{6.8139e-11}$ \hspace{1cm} & slightly modified universal gravitational constant \\ \hline
    $m_1 = 1.00$ & mass of a bowling ball (test mass) \\ \hline
    $m_2 = \num{5.97e24}$ & mass of Earth \\ \hline
    $R = \num{6.378e6}$ & radius of Earth \\ \hline
    $h = 0 $ & height of $m_1$ above Earth's surface (initially zero) \\ \hline
    $\displaystyle E_i = -\frac{G m_1 m_2}{R}$ & gravitational potential energy when $m_1$ is on Earth's surface \\ \hline
    $\displaystyle E_f = - \frac{G m_1 m_2}{R+h}$ & gravitational potential energy when $m_1$ is raised to height $h$ \\ \hline
    $E_{\mathrm{change}} = E_f - E_i$ & the \textit{exact} change in gravitational potential energy from $E_i$ to $E_f$ \\ \hline
\end{tabular}
\end{center}
\egroup

%Note: \footnote{Setting $G = \num{6.814e-11}$ makes $g = 10$.}
...
\textbf{\underline{NOTE}:} For all answers below, \textbf{\underline{ROUND}} to the nearest whole number.

\begin{parts}
\part 
\textbf{\underline{Set}} the object position to $h = 0$, and \textbf{\underline{list}} the initial, final, and change in gravitational potential energies below:
...
\begin{tabular}{|c|c|c|}
    \hline 
     ...
     \ifprintanswers \color{red} $-63780155$ \fi & \ifprintanswers \color{red} $-63780155$ \fi & \ifprintanswers \color{red} $0$ \fi \\ \hline
\end{tabular}
\egroup
\end{center}

\part 
\textbf{\underline{Set}} $h = 1$, and \textbf{\underline{list}} the energies below.
...
\begin{tabular}{|c|c|c|}
    \hline 
     \hspace{2.0cm} $E_i$ \hspace{2.0cm} & \hspace{2.0cm} $E_f$ \hspace{2.0cm} & \hspace{1cm} $E_\mathrm{change}$ \hspace{1.5cm} \phantom{.}\\ \hline
     \ifprintanswers \color{red} $-63780155$ \fi & \ifprintanswers \color{red} $-63780145$ \fi & \ifprintanswers \color{red} $10$ \fi \\ \hline
\end{tabular}
\egroup
\end{center}

\part \textbf{\textit{}}
\textbf{\underline{Set}} $h = 2$, and \textbf{\underline{list}} the energies below.
...
\begin{tabular}{|c|c|c|}
    \hline 
     \hspace{2.0cm} $E_i$ \hspace{2.0cm} & \hspace{2.0cm} $E_f$ \hspace{2.0cm} & \hspace{1cm} $E_\mathrm{change}$ \hspace{1.5cm} \phantom{.}\\ \hline
     \ifprintanswers \color{red} $-63780155$ \fi & \ifprintanswers \color{red} $-63780135$ \fi & \ifprintanswers \color{red} $20$ \fi \\ \hline
\end{tabular}
\egroup
\end{center}
...
\part 
\textbf{\underline{Change}} test mass $m_1$ and vertical position $h$ according to the following tables, and \textbf{\underline{record}} the change in gravitational potential energy, $E_\mathrm{change}$, in each case.
...
\part \textit{Synthesis.} Do you see the pattern in your data table above? \textbf{\underline{Create}} an equation for the change in gravitational potential energy $\Delta E_g$ in terms of mass $m_1$, height $h$, and any additional constants.

\bgroup
\Large
\begin{equation*}
    \Delta E_g = \ifprintanswers \color{red} \else \color{white} \fi mh \times 10
\end{equation*}
\egroup

\part
\textbf{\underline{Express}} the equation you created in the previous step, in words. Use the sentence stem below.

\ifprintanswers
{\color{red} The change in gravitational potential energy equals mass multiplied by height multiplied by 10.
...
\textit{The change in gravitational potential energy equals}
\vspace{-1.5em}
\fillwithlines{14mm}
\fi

\part
What physical quantity do you think the factor of 10 represents?

\ifprintanswers
\textcolor{red}{``In the past, 10 has been associated with $g = \SI{10}{N/kg} = \SI{10}{m/s^2}$, the strength of the gravitational field or gravitational acceleration, so I think the 10 is $g$. Therefore, $\Delta E_g = mgh$.''}
\else
\fillwithlines{14mm}
\fi
...
\question
\textit{Change in GPE relative to a reference line.} Let $h_0$ be a variable reference point above Earth's surface from which we may measure a change in gravitational potential energy. 
...
For example, if the reference point is at sea level, then $h_0 = \SI{0}{m}$. If it's at the top of a 100-meter building, then $h_0 = \SI{100}{m}$, etc. 

In Desmos, \textbf{\underline{set}} $m_1 = 1$ and  \textbf{\underline{update}} the cells for the initial and final gravitational potential energies:
...
\begin{tabular}{|c|l|}
    \hline
    \thead{\textbf{Update these cells in Desmos}} & \thead{\textbf{What it means}} \\ \hline
    $\displaystyle E_i = -\frac{G m_1 m_2}{R + h_0}$ & gravitational PE starting from reference $h_0$ \\ \hline
    $\displaystyle E_f = - \frac{G m_1 m_2}{R + h_0 + h}$ & gravitational PE ending at $h_0 + h$ \\ \hline
\end{tabular}
\end{center}
\egroup


Houston is 15 meters above sea level, and the tallest building in Houston, the JP Morgan Chase Tower, is 305 meters tall. So, the top of the Chase Tower is 320 meters above sea level. Imagine standing on top of the tower and raising the bowling ball ($m_1$) to a height of 10 meters relative to the reference line.

\begin{center}
\begin{tikzpicture}
    \draw[rounded corners=2mm] (0,0) -- (3,0)  -- (5,1) -- (9,1) node[above,pos=0.3,align=left] {Houston\\ground level};
    \draw[densely dashed] (3,0) -- (8.9,0) node[above,pos=0.5] {Sea level} node[below=5mm,pos=0.5] {\footnotesize Note: Figure is not to scale.};
    \draw[|-|] (9.5,0) -- (9.5,5) node[pos=0.8,right,align=left] {$h_0 = \SI{320}{m}$\\reference height};
    \fill (8.8,6) circle (1.5mm) node[left=1.5mm] {$m_1$};
    \draw[thick,fill=gray] (8,1) rectangle (9,5) node[rotate=90,black,pos=0.5] {\bfseries \large Chase Tower};
    \draw[|-|] (9.15,5) -- (9.15,6) node[right,pos=0.5] {$h = \SI{10}{m}$};
    \node at (8.5,5.2) {\Strichmaxerl[1.5]};
    \begin{pgfonlayer}{background}
        \fill[cyan!20] (0,-1) rectangle (3,0) node[pos=0.5] {\reflectbox{\twemoji[width=6mm]{fish}}};
    \end{pgfonlayer}
\end{tikzpicture}
\end{center}
...
Make the following changes in Desmos:
...
\begin{tabular}{|c|l|}
    \hline
    \thead{\textbf{Update these cells in Desmos}} & \thead{\textbf{What it means}} \\ \hline
    $\displaystyle h_0 = 320$ & the new reference point is at the top of the Chase Tower \\ \hline
    $\displaystyle h = 10$ & raise the bowling ball 10 meters above the reference \\ \hline
\end{tabular}
\end{center}
\egroup
...
\begin{parts}
\part 
The \textit{exact} change in gravitational potential energy (PE) for this configuration is given by the cell containing $E_\mathrm{change}$ in Desmos. What is the change in gravitational PE? (Round to nearest whole.)

\begin{center}
    $E_\mathrm{change} = $ \fillin[100] \quad joules
\end{center}

\part
What is the change in gravitational potential energy according to the formula $\Delta E_g = m g h$?

\ifprintanswers
\textcolor{red}{$\Delta E_g = mgh = (\SI{1}{kg})(\SI{10}{m/s^2})(\SI{10}{m}) = \SI{100}{J}$, so also 100 joules.}
\else
\fillwithlines{7mm}
\fi

\part 
Is $mgh$ a good representation of the exact value $E_\mathrm{change}$? Why or why not?

\ifprintanswers
\textcolor{red}{Yes because $E_\mathrm{change}$ in Desmos and $mgh$ both give the same answer of 100 joules.}
\else
\fillwithlines{14mm}
\fi
...
\question
Mount Everest is \SI{8850}{m} above sea level ($h_0 = 8850$).

\begin{parts}
\part What is the \textit{exact} change in gravitational potential energy when the 1-kg ball is raised 10 meters above Mount Everest? (Round to the nearest whole number.)

\ifprintanswers
\textcolor{red}{$E_\mathrm{change} = 100$}
\else
\fillwithlines{7mm}
\fi
...
\ifprintanswers
\textcolor{red}{Since $\Delta E_g = mgh = (\SI{1}{kg})(\SI{10}{m/s^2})(\SI{10}{m}) = \SI{100}{J}$, the equation $mgh$ is a good representation of the exact value.}
\else
\fillwithlines{14mm}
\fi
...
\question
The International Space Station (ISS) orbits 400 kilometers above the surface of the Earth ($h_0 = \num{400000}$).

\begin{parts}
    
\part What is the \textit{exact} change in gravitational potential energy when the 1-kg ball is raised 10 meters above the ISS? (Round to the nearest whole number.)

\ifprintanswers
\textcolor{red}{$E_\mathrm{change} = 89$}
\else
\fillwithlines{7mm}
\fi
...
\ifprintanswers
\textcolor{red}{No. The true value is $E_\mathrm{change} = 89$ but $\Delta E_g = mgh$ still predicts \SI{100}{J}, so equation $mgh$ is a no longer a good representation of the exact value.}
\else
\fillwithlines{14mm}
\fi
\end{parts}

\question \textit{Checking for understanding.} The expression $\Delta E_g = mgh$ is a good approximation for the \textit{exact} change in the gravitational potential energy\ldots

\begin{randomizechoices}[norandomize]
    \choice only if the reference line is at sea level.
    \choice only if the reference line is below the International Space Station. 
    \correctchoice only if the reference line is sufficiently near sea level (e.g., below Mount Everest).
    \choice only if the reference line is far from sea level (e.g., above the ISS).
    \choice at any distance beyond the Earth's surface.
...
\subsection*{Demos: Mechanical Energy}

\begin{questions}
\question
A 1-kilogram bowling ball is launched vertically upwards at \SI{18}{m/s}. During a time interval of 3.6 seconds, the ball reaches the apex and travels back to the launch point. Below you'll \textbf{\underline{make}} the following graphs:

\begin{itemize}[itemsep=0pt]
    \item kinetic energy vs. time ($K$ vs.\:$t$)
    \item gravitational potential energy vs. time ($P$ vs.\:$t$), assuming the launch point is the zero line
    \item mechanical energy vs. time ($E$ vs.\:$t$)
\end{itemize}

\textbf{\underline{Access}} the Desmos graphing calculator at \href{https://www.desmos.com/calculator}{desmos.com/calculator}. \textbf{\underline{Type}} the following variables and equations in each cell:
...
\centering
\begin{tikzpicture}
    \draw[ultra thick,->] (0,0) -- (0,1) node[left,pos=0.8] {$v_0$};
    \draw[densely dashed] (0,0) node[below,align=center] {launch\\point} -- (0,6.1) node[above] {apex};
    \fill (0,4) circle (2mm) node[right=2mm] {\SI{1}{kg}};
\end{tikzpicture}
\end{minipage}%
\begin{minipage}{13cm}
\centering
...
\begin{tabular}{|l|l|}
    \hline
    \thead{\textbf{Type this in Desmos}} & \thead{\textbf{What it means}} \\ \hline
    $m = 1$ & mass of launched object \\ \hline
    $g = 10 $ & magnitude of gravitational acceleration \\ \hline
    $v_0 = 18$ & vertical launch speed \\ \hline
    $v(t) = v_0 - gt$ & vertical velocity as a function of time $t$ \\ \hline
    $K(t) = \frac{1}{2} m \left(v\left(t\right)\right)^2 $ & kinetic energy (KE) as a function of time $t$ \\ \hline
    $h(t) = v_0 t - \frac{1}{2}g t^2$ & height as a function of time $t$ \\ \hline
    $P(t) = mg h(t)$ & gravitational potential energy (PE) as a function of time $t$ \\ \hline
    $E(t) = K(t) + P(t) $ & mechanical energy (ME) as a function of time $t$ \\ \hline
\end{tabular}
\end{minipage}
\end{EnvUplevel}
...
\textbf{\underline{Disable}} the graphs for $v(t)$ and $h(t)$ by clicking the left margin of each cell so that only $K(t)$, $P(t)$, and $E(t)$ are visible. Next, \textbf{\underline{click}} the graph settings (\faWrench) and set the X-Axis and Y-Axis limits to
...
Finally, \textbf{\underline{sketch}} the kinetic energy, gravitational potential energy, and mechanical energy vs. time graphs in the space below.
...
\begin{axis}[height=7cm,width=8cm,
    ...
    \ifprintanswers
    \addplot[very thick,domain=0:3.6,variable=t,red] {0.5*1*(18-10*t)^2};
    \addplot[very thick,domain=0:3.6,variable=t,blue] {1*10*(18*t-0.5*10*t^2)};
    \addplot[very thick,domain=0:3.6,variable=t] {0.5*1*(18-10*t)^2 + 1*10*(18*t-0.5*10*t^2)};
    \legend{KE, GPE, ME}
    \fi
\end{axis}
\end{tikzpicture}
...
\subsection*{PhET Simulation: Energy Skate Park [UNDER CONSTRUCTION]}



%Today, you will use the Energy Skate Park Lab to explore energy transformations and determine how the position of a skater affects his speed, potential energy and kinetic energy. http://bit.ly/3ucY9zW

\textbf{Beginning Observations}

\begin{questions}

\question
Open the Energy Skate Park PhET Simulation. What can you change about the scenario within the simulation? 

\begin{solution}
    Mass, friction and gravity, speed, track
\end{solution}

\question
There are two types of energy representations available. Open both. Draw or describe the two types.
...
\question
Select the track of your choice. Select “Pie Chart”  and click on  .   What storage forms of energy are shown in the graph and chart? List below as well as the definition of each.

\begin{solution}
- Kinetic (Energy due to motion); Potential (Energy due to position); Thermal (energy due to heat) and Total Mechanical (sum of PE and KE)
\end{solution}


\uplevel{\bfseries Part 1: Energy Transformations }

\question
Create your own track or select one of the pre-made tracks. make sure to turn off friction. Draw four points on your track that you would like to investigate.  Place a screenshot of your track below.

\question
Fill in the predictions column by sketching what you think the Energy pie chart will look like for the ball at your four points. Then, use the simulation to test your ideas and explain any differences. 
Point	Prediction	Simulation	Explain Differences

\begin{solution}
1	
 	No difference, or just a little KE since it’s difficult to pause at the exact point.
2	
 	Almost the same, more KE than PE since closer to the valley.
3	
 	Equal amounts of PE and KE since 3 is half way down from 1.
4	
 	Very similar. More PE since we went past the middle path.
\end{solution}

\question
The Law of Conservation of Energy states that energy cannot be created or destroyed. Therefore, energy can only be transformed. Use this statement to answer the questions that follow.
a) What type(s) of energy did the skater have at the skater’s highest point? 
...
\question    
What type(s) of energy did the skater at his lowest point?
...
\question   
What type of energy transformation did you see between the highest and lowest point in the motion of the skater? 

\begin{solution}
    The PE was being transformed to KE
\end{solution}

\question
What is the definition of mechanical energy? Was mechanical energy conserved?

\begin{solution}
    Sum of KE and PE in a system. Yes, the TME stayed the same at any of the 4 points.
\end{solution}

\question
turn on friction and redo your predictions. then run the simulation again and fill out the table below
Point	Prediction	Simulation	Explain Differences
...
1	
 	No difference
2	
 	Had not experienced friction long enough as I thought.
3	
 	No difference. Equal amounts of PE and KE, while thermal is less than both.
4	
 	No Difference
\end{solution}

\question
Did adding friction allow for conservation of mechanical energy? Did this violate the law of conservation of energy? why or why not.

\begin{solution}
    No. Because as time went on, the sum of PE and KE does not give us the total. This indicate that some of the energy is lost as heat. However, this did not violate the law since the total energy stayed constant.
\end{solution}

\uplevel{\bfseries Part 2: Relating Speed to Potential and Kinetic Energy}

\question
Label on the track where you predict the skater will is 

a) at his maximum speed
b) stopped
c) going his average speed
d) going slow
e) going fast

\question
On the graph to the right label where you predict the skater is 
a) at his maximum speed
b) stopped
c) going his average speed
d) going slow
...
\question
Test your ideas using the Double Well Roller Coaster track. 
a) Make any modifications to your answers in RED on the images for Questions 4 and 5. 

\question
If one of your friends asked you for help in making sense of the type of graph in Question 5. How would you explain what it is showing? *Consider: What is on each axis?  What types of energy are being graphed?

\begin{solution}
    The graph shoes the transformation between KE and PE with position and indicates that as one decreases, the other increases with an equal amount, thus keeping the TME conserved. There is more PE at higher elevation positions and more KE at positions with lower elevation.
\end{solution}

\question
Using the graph in Question 5, what can you say about the total energy of the skater? 

\begin{solution}
      It stays the same all the time. 
\end{solution}

\uplevel{\bfseries Post Lab}

\question
The diagram below shows a frictionless track. 

Assume a skater were to complete the track. At which positions is the sum of the potential and kinetic energy the same? Explain. 

\begin{solution}
    At every position, since there is only a transformation from one into the other.
\end{solution}

\question
Using the diagram in Question 7, assume that the skater moves from point 1 to point 2. What type of energy transformation would exist in the movement between those two points?

\begin{solution}
    As the skater moves from point 1-2, their PE continually decreases while their KE continually increases.
\end{solution}
\end{questions}
...
\subsection*{Lab: Momentum and Collisions}

\begin{mdframed}
\textbf{Setting up the collision lab.}
\begin{enumerate}[topsep=0pt]
    \item Set up the Dynamics Track so that it reads \SI{0}{cm} to \SI{121}{cm} from left to right.
    \item Place the two carts on the track, with the $+x$ arrow pointing towards \SI{120}{cm}.
    ...
    \begin{tikzpicture}
        \begin{axis}[width=15cm,
            ...
            xtick={0,10,...,120},
            minor x tick num=9]
            \draw[thick] (5,0) rectangle ++(20,1cm) node[pos=0.5,align=center] {Cart 1\\$\rightarrow +x$};
            \draw[thick] (60,0) rectangle ++(20,1cm) node[pos=0.5,align=center] {Cart 2\\$\rightarrow +x$};
        \end{axis}
    \end{tikzpicture}
    \end{center}

    \item Label the left one \textit{Cart 1} and the other \textit{Cart 2}. Do \textbf{\underline{NOT}} change their order or direction.
    \item Launch Graphical Analysis at \href{https://graphicalanalysis.app/}{graphicalanalysis.app/}.
    \begin{itemize}
        \item Click ``Sensor Data Collection'' and connect with ``Wireless'' using the Carts' barcode ID.
        \item Enable ``Light Mode'' in the app preferences.
        \item Select the ``1 Graph'' option, and change the $y$-axis to Velocity.
        \item \textit{Test cart direction.} Press ``Collect'' and move each cart from left to right. Their velocities should be positive. If velocities are negative, reverse the axis in the app settings.
    \end{itemize}
    \item Locate the magnetic tabs on a cart's bumper. Practice reversing the magnets so that the carts stick together (inelastic collision) or repel each other (elastic collision).
\end{enumerate}
\textbf{\underline{Note}}: In the tables below, record \textbf{\underline{ALL}} values to at least 3 decimal places (e.g., \SI{0.391}{m/s}).
\end{mdframed}

\textbf{Inelastic Collisions}

\textbf{\underline{Setup}:} For an inelastic collision, set the magnets so that the carts stick together. 

\begin{questions}
\question \label{nLPqE}
\textit{Inelastic collision with Cart 2 initially at rest}. 

\begin{parts}
\part Measure the mass of Carts 1 and 2. Convert to kilograms (e.g., \SI{213}{g} equals \SI{0.213}{kg}), and record values in the table below.


\part
Place Cart 2 at rest in the middle of the track, and Cart 1 near \SI{0}{cm}. Click ``Collect'' and push Cart 1 so it rolls toward Cart 2. The carts will collide, stick together, and roll together.
...
\begin{tikzpicture}
    \begin{axis}[width=15cm,
        ...
        xtick={0,10,...,120},
        minor x tick num=9]
        \draw (5,0) rectangle ++(20,8mm) node[pos=0.5] {Cart 1 ($m_1$)};
        \draw[very thick,->] (27,4mm) -- ++(10,0) node[pos=0.5,above] {$\vec{v}_{1i}$};
        \draw (60,0) rectangle ++(20,8mm) node[pos=0.5,align=center] {Cart 2 ($m_2$)} node[pos=0.5,right=11] {$\vec{v}_{2i} = 0$};
    \end{axis}
    \node[above=1mm] at (current bounding box.north) {\bfseries Before Collision};
\end{tikzpicture}
...
\begin{tikzpicture}
    \begin{axis}[width=15cm,
        ...
        xtick={0,10,...,120},
        minor x tick num=9]
        \draw (45,0) rectangle ++(20,8mm) node[pos=0.5] {Cart 1};
        \draw (65,0) rectangle ++(20,8mm) node[pos=0.5,align=center] {Cart 2};
        \draw[very thick,->] (88,4mm) -- ++(10,0) node[pos=0.5,above] {$\vec{v}_f$};
    \end{axis}
    \node[above=1mm] at (current bounding box.north) {\bfseries After Collision};
\end{tikzpicture}
\end{EnvUplevel}


\part 
Collect velocity data from graphical analysis and record values in the data table below.\\(See part (d) below for guidance on calculating momentum values.)

\begin{EnvUplevel}
\centering
\def\arraystretch{2}
%...This table was powered by ChatGPT.
\begin{tabular}{|c|c|c|c|c|c|c|c|c|}
    \hline
    \thead{Run\\number} 
    & \thead{Cart ID} 
    & \thead{Mass of\\cart\\(kg)} 
    & \thead{Velocity\\before\\collision\\(m/s)} 
    & \thead{Velocity\\after\\collision\\(m/s)} 
    & \thead{Momentum\\before\\collision\\(\SI{}{kg\cdot m/s})} 
    & \thead{Momentum\\after\\collision\\(\SI{}{kg\cdot m/s})} 
    & \thead{Total\\momentum\\before\\collision\\(\SI{}{kg\cdot m/s})} 
    & \thead{Total\\momentum\\after\\collision\\(\SI{}{kg\cdot m/s})} \\ 
    \hline

    \multirow{2}{*}{1}
      & Cart 1
      & {\anscolor \hspace{2mm} 0.29242 \hspace{2mm}} & {\anscolor 0.388} & \multirow{2}{*}{{\anscolor 0.195}}
      & {\anscolor 0.1135} & \multirow{2}{*}{{\anscolor 0.1145}}
      & \multirow{2}{*}{{\anscolor 0.1135}}
      & \multirow{2}{*}{{\anscolor 0.1145}} \\
    \cline{2-4}\cline{6-6}
      & Cart 2
      & {\anscolor 0.29486} & {\anscolor 0.000} &
      & {\anscolor 0.000} & & & \\ 
    \hline
\end{tabular}
\end{EnvUplevel}

\part Calculate momentum ($p=mv$) values and record them in the table above, as follows:

\begin{align*}
    m_1 \text{ momentum before collision} &= m_1 \vec{v}_{1i} \\[1ex]
    m_2 \text{ momentum before collision} &= m_2 \vec{v}_{2i} \\[1ex]
    \text{total momentum before collision} &= m_1 \vec{v}_{1i} + m_2 \vec{v}_{2i} \\[1ex]
    \text{total momentum after collision} &= (m_1 + m_2) \vec{v}_f
\end{align*}
...
\question
\textit{Inelastic collision with Cart 2 initially in motion}.

\begin{parts}
\part Copy mass values to the table below from the previous table. 

\part
Place the Cart 2 in the middle of the track, and Cart 1 near \SI{0}{cm}. Click ``Collect'' and release both Carts 1 and 2 to roll to the right with Cart 2 at a slower velocity, so they collide.
...
\begin{tikzpicture}
    \begin{axis}[width=15cm,
        ...
        xtick={0,10,...,120},
        minor x tick num=9]
        \draw (5,0) rectangle ++(20,8mm) node[pos=0.5] {Cart 1};
        \draw[very thick,->] (27,4mm) -- ++(10,0) node[pos=0.5,above] {$\vec{v}_{1i}$};
        \draw (60,0) rectangle ++(20,8mm) node[pos=0.5,align=center] {Cart 2};
        \draw[very thick,->] (82,4mm) -- ++(5,0) node[pos=0.5,above] {$\vec{v}_{2i}$};
    \end{axis}
    \node[above=1mm] at (current bounding box.north) {\bfseries Before Collision};
\end{tikzpicture}
...
\part 
Collect velocity data from graphical analysis and record values in the data table below.

\part
Calculate momentum ($p=mv$) values and record them in the table.
...
\begin{tabular}{|c|c|c|c|c|c|c|c|c|}
    ...
    \multirow{2}{*}{2}
      & Cart 1
      & {\anscolor \hspace{2mm} 0.29242 \hspace{2mm}} & {\anscolor 0.450} & \multirow{2}{*}{{\anscolor 0.300}}
      & {\anscolor 0.1316} & \multirow{2}{*}{{\anscolor 0.1762}}
      & \multirow{2}{*}{{\anscolor 0.1761}}
      & \multirow{2}{*}{{\anscolor 0.1762}} \\
    \cline{2-4}\cline{6-6}
      & Cart 2
      & {\anscolor 0.29486} & {\anscolor 0.151} &
      & {\anscolor 0.04452} & & & \\ 
    \hline
\end{tabular}
\end{EnvUplevel}
...
\question
\textit{Inelastic collision with mass added to Cart 2}.

\begin{parts}
\part
Add two weights to Cart 2. Measure the new mass of Cart 2, and update mass values in the table below.

\part
Place Cart 2 at rest in the middle of the track, and Cart 1 near \SI{0}{cm}. Click ``Collect'' and push Cart 1 so it rolls toward Cart 2, like in Question \ref{nLPqE}(b) above.

\part 
Collect velocity data from graphical analysis and record values in the data table below.

\part Calculate momentum ($p=mv$) values and record them in the table.
...
\begin{tabular}{|c|c|c|c|c|c|c|c|c|}
    ...
    \multirow{2}{*}{3}
      & Cart 1
      & {\anscolor \hspace{2mm} 0.29242 \hspace{2mm}} & {\anscolor 0.480} & \multirow{2}{*}{{\anscolor 0.172}}
      & {\anscolor 0.1404} & \multirow{2}{*}{{\anscolor 0.1439}}
      & \multirow{2}{*}{{\anscolor 0.1404}}
      & \multirow{2}{*}{{\anscolor 0.1439}} \\
    \cline{2-4}\cline{6-6}
      & Cart 2
      & {\anscolor 0.54416} & {\anscolor 0.000} &
      & {\anscolor 0.000} & & & \\ 
    \hline
\end{tabular}
\end{EnvUplevel}
...
\textbf{\underline{Setup}:} For an elastic collision, set the magnets so that the carts bounce away from each other. 
\end{EnvUplevel}


\question \label{HrRIK}
\textit{Elastic collision with Cart 2 initially at rest}. 

\begin{parts}
\part 
Copy the original mass values for Carts 1 and 2 to the table below.

\part
Place Cart 2 at rest in the middle of the track, and Cart 1 near \SI{0}{cm}. Click ``Collect'' and gently push Cart 1 so it rolls toward Cart 2. The carts will collide and bounce off each other.
...
\begin{tikzpicture}
    \begin{axis}[width=15cm,
        ...
        \draw (5,0) rectangle ++(20,8mm) node[pos=0.5] {Cart 1};
        \draw[very thick,->] (27,4mm) -- ++(10,0) node[pos=0.5,above] {$\vec{v}_{1i}$};
        \draw (60,0) rectangle ++(20,8mm) node[pos=0.5,align=center] {Cart 2} node[pos=0.5,right=11] {$\vec{v}_{2i} = 0$};
    \end{axis}
    \node[above=1mm] at (current bounding box.north) {\bfseries Before Collision};
\end{tikzpicture}
...
\begin{tikzpicture}
    \begin{axis}[width=15cm,
        ...
        minor x tick num=9]
        \draw (45,0) rectangle ++(20,8mm) node[pos=0.5] {Cart 1};
        \draw[very thick,->] (67,4mm) -- ++(10,0) node[pos=0.5,above] {$\vec{v}_{1f}$};
        \draw (85,0) rectangle ++(20,8mm) node[pos=0.5,align=center] {Cart 2};
        \draw[very thick,->] (107,4mm) -- ++(10,0) node[pos=0.5,above] {$\vec{v}_{2f}$};
    \end{axis}
    \node[above=1mm] at (current bounding box.north) {\bfseries After Collision};
\end{tikzpicture}
\end{EnvUplevel}
...
\part Calculate momentum ($p=mv$) values and record them in the table. This time,

\begin{equation*}
    \text{total momentum after collision} = m_1 \vec{v}_{1f} + m_2 \vec{v}_{2f}
\end{equation*}
...
\begin{tabular}{|c|c|c|c|c|c|c|c|c|}
    ...
    \multirow{2}{*}{4}
      & Cart 1
      & {\anscolor \hspace{2mm} 0.29242 \hspace{2mm}} & {\anscolor 0.342} & {\anscolor 0.000}
      & {\anscolor 0.1000} & {\anscolor 0.000}
      & \multirow{2}{*}{{\anscolor 0.1000}}
      & \multirow{2}{*}{{\anscolor 0.09907}} \\
    \cline{2-7}
      & Cart 2
      & {\anscolor 0.29486} & {\anscolor 0.000} & {\anscolor 0.336}
      & {\anscolor 0.000} & {\anscolor 0.09907}
      & & \\ 
    \hline
\end{tabular}
\end{EnvUplevel}
...
\question
\textit{Elastic collision with Cart 2 initially in motion}.

\begin{parts}
\part Copy mass values to the table below from the previous table. 

\part
Place Cart 1 near \SI{0}{cm} and Cart 2 on the opposite end. Click ``Collect'' and release both Carts 1 and 2 to roll towards each other, so that the carts move in opposite directions after the collision.
...
\begin{tikzpicture}
    \begin{axis}[width=15cm,
        ...
        xtick={0,10,...,120},
        minor x tick num=9]
        \draw (15,0) rectangle ++(20,8mm) node[pos=0.5] {Cart 1};
        \draw[very thick,->] (37,4mm) -- ++(10,0) node[pos=0.5,above] {$\vec{v}_{1i}$};
        \draw (85,0) rectangle ++(20,8mm) node[pos=0.5,align=center] {Cart 2};
        \draw[very thick,->] (83,4mm) -- ++(-10,0) node[pos=0.5,above] {$\vec{v}_{2i}$};
    \end{axis}
    \node[above=1mm] at (current bounding box.north) {\bfseries Before Collision};
\end{tikzpicture}
...
\begin{tikzpicture}
    \begin{axis}[width=15cm,
        ...
        xtick={0,10,...,120},
        minor x tick num=9]
        \draw (35,0) rectangle ++(20,8mm) node[pos=0.5] {Cart 1};
        \draw[very thick,->] (33,4mm) -- ++(-10,0) node[pos=0.5,above] {$\vec{v}_{1f}$};
        \draw (65,0) rectangle ++(20,8mm) node[pos=0.5,align=center] {Cart 2};
        \draw[very thick,->] (87,4mm) -- ++(10,0) node[pos=0.5,above] {$\vec{v}_{2f}$};
    \end{axis}
    \node[above=1mm] at (current bounding box.north) {\bfseries After Collision};
\end{tikzpicture}
\end{EnvUplevel}
...
\begin{tabular}{|c|c|c|c|c|c|c|c|c|}
    ...
    \multirow{2}{*}{5}
      & Cart 1
      & {\anscolor \hspace{2mm} 0.29242 \hspace{2mm}} & {\anscolor 0.677} & {\anscolor 0.254}
      & {\anscolor 0.1980} & {\anscolor 0.07427}
      & \multirow{2}{*}{{\anscolor 0.2740}}
      & \multirow{2}{*}{{\anscolor 0.2709}} \\
    \cline{2-7}
      & Cart 2
      & {\anscolor 0.29486} & {\anscolor 0.258} & {\anscolor 0.667}
      & {\anscolor 0.07607} & {\anscolor 0.1967}
      & & \\ 
    \hline
\end{tabular}
\end{EnvUplevel}
...
\question
\textit{Elastic collision with mass added to Cart 2}.
...
\part
Place Cart 2 at rest in the middle of the track, and Cart 1 near \SI{0}{cm}. Click ``Collect'' and push Cart 1 so it rolls toward Cart 2. The carts will collide and bounce off each other, like in Question \ref{HrRIK}(b). 
...
\begin{tabular}{|c|c|c|c|c|c|c|c|c|}
    ...
    \multirow{2}{*}{6}
      & Cart 1
      & {\anscolor \hspace{2mm} 0.29242 \hspace{2mm}} & {\anscolor 0.401} & {\anscolor $-0.107$}
      & {\anscolor 0.1173} & {\anscolor $-0.03129$}
      & \multirow{2}{*}{{\anscolor 0.1173}}
      & \multirow{2}{*}{{\anscolor 0.1200}} \\
    \cline{2-7}
      & Cart 2
      & {\anscolor 0.54416} & {\anscolor 0.000} & {\anscolor 0.278}
      & {\anscolor 0.000} & {\anscolor 0.1513}
      & & \\ 
    \hline
\end{tabular}
\end{EnvUplevel}
...
\question
\textit{Analysis.}
If the total momentum for a system is the same before and after the collision, we say that momentum is conserved. Let's look for evidence of momentum conservation.

Look at the last 2 columns of each of the tables above. For the two-cart system, let

\begin{align*}
    p_i &= \text{total momentum before collision} \\
    p_f &= \text{total momentum after collision}
\end{align*}
...
\begin{equation*}
    \text{percent difference} = \frac{\left|p_f - p_i\right|}{\frac{1}{2}\left(\left|p_i\right| + \left|p_f\right|\right)} \times 100
\end{equation*}

to calculate the percent difference in total momentum for each of the 6 trials, and record your values below. \textit{Tip:} Define variables in \href{https://www.desmos.com/scientific}{desmos.com/scientific} to streamline your calculations.

\begin{center}
\def\arraystretch{2}
\begin{tabular}{|c|c|c|c|c|c|c|}
    \hline
     & Run 1 & Run 2 & Run 3 & Run 4 & Run 5 & Run 6 \\ \hline
    \% difference & {\anscolor 0.93} & {\anscolor 0.04} & {\anscolor 2.5} & {\anscolor 0.94} & {\anscolor 1.1} & {\anscolor 2.3} \\ \hline
\end{tabular}
\end{center}

\part If the percent difference is less than 5\%, there is good evidence that momentum is conserved. Was momentum conserved in the 6 runs in this lab? Cite evidence from the table above.
...
%     1 & Cart 1 & & & \multirow{2}{*}{} & & & & \\ \cline{1-4}\cline{6-9}
%     1 & Cart 2 & & &                 & & & & \\ \hline
...
%     & \thead{Momentum\\after\\collision\\(\SI{}{kg\cdot m/s})} 
%     & \thead{Total\\momentum\\before\\collision\\(\SI{}{kg\cdot m/s})} 
%     & \thead{Total\\momentum\\after\\collision\\(\SI{}{kg\cdot m/s})}  \\ \hline
%     1 & Cart 1 & & & & & & & \\ \hline
%     1 & Cart 2 & & & & & & & \\ \hline
%     2 & Cart 1 & & & & & & & \\ \hline
%     2 & Cart 2 & & & & & & & \\ \hline
