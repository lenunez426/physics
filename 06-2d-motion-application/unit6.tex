...
% \title{Unit 6: Two-Dimensional Motion}
% \author{Physics\footnote{Access for free at \href{\openstax}{\openstax}} \hspace{0.1ex} at CFISD}
...
Take a look at Figure \ref{xXmnIC}. It's possible to find the magnitude of vector $A$ if you know the angle $\theta$ and either the $x$- or $y$-component. Vector $A$ is related to  angle $\theta$ and its components by the trig functions cosine (cos) and sine (sin), as follows:
...
\subsection*{\ref{tMF7JY} Exercises}
...
\begin{axis}[
    ...
    % \draw[red] (-0.7,-0.4) rectangle (1,1.5);
    \draw[gray] (0,0) circle (1);
    \fill (0,0) circle (1pt);
    \fill ({cos(45)},{sin(45)}) circle (3pt) node[right=2pt] {$m$};
    \begin{scope}[xshift=0.12cm,yshift=-0.12cm]
        \draw[dashed] (0,0) node[rotate=45] {\tiny $|$} -- ({cos(45)},{sin(45)}) node[rotate=45] {\tiny $|$};
        \node at (0.4,0.2) {$r$};
    \end{scope}
    \draw[ultra thick,->] ({cos(45)},{sin(45)}) -- (0.3,0.3) node[left] {$a_{\text{c}}$};
    \draw[thick,->] ({cos(45)},{sin(45)}) -- ++(axis direction cs: -0.6,0.6) node[left] {$v$};
\end{axis}
\end{tikzpicture}
\end{center}

\begin{mdframed}[backgroundcolor=black!10]
\Gls{centripetal acceleration} is the acceleration of an object moving in a circle, directed toward the center of the circle. The equation for centripetal acceleration is
...
Recall that Newton's Second Law of Motion relates force, mass, and acceleration:

\begin{equation*}
    F_{\text{net}} = ma
\end{equation*}

Therefore, if we know the mass of the object, we can multiply it by its centripetal acceleration to get a centripetal force.

\begin{mdframed}[backgroundcolor=black!10]
\Gls{centripetal force} is any force causing uniform circular motion. This force is in the direction of the centripetal acceleration: towards the center of the circle.

\begin{equation} \label{qzzym8}
    F_{\text{c}} = m a_{\text{c}} = \frac{m v^2}{r}
\end{equation}
...
\subsection*{\ref{uPOucv} Exercises}
\begin{multicols}{2}
...
\begin{exercise} \label{fsjtBV}
    Select the quantity that is constant in uniform circular motion: speed, velocity, acceleration, displacement.
\end{exercise}

\begin{exercise} \label{GD8Yff}
    List all the quantities that influence centripetal force.
\end{exercise}
...
%         \draw[<->] (0,0.2) -- (5,0.2);
%         \node at (2.5,0.4) {$r$};
%         \draw (0,0) circle (0.5cm) node[above,inner sep=0.5cm] {\SI{120}{kg}};
%         \draw (5,0) circle (0.7cm) node[above,inner sep=0.7cm] {\SI{434}{kg}};
%         \draw[->,thick] (0,0) -- (2,0);
%         \draw[->,thick] (5,0) -- (3,0) ;
%         \node at (1.5,-0.3) {\SI{8.0e-7}{N}};
%         % \node at (3.8,-0.25) {$F$};
%     \end{tikzpicture}
% \end{center}
